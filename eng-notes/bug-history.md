...
## 20. 加新 MCP 工具后 tg 端调不到（长 claude 进程攥着旧工具列表）

**症状**: `server.py` 里加了新工具（ebook 共读 + 弹幕那批），`systemctl restart exec-mcp` 也重启了、表也建好了，但 tg 端小宝就是调不到新工具。新开的 Code 窗口却能正常用。

**根因**: tg bot **不**直接 spawn `claude -p`，而是走 `claude-chat-daemon.service`（`/opt/claude-chat-daemon/chat_daemon.py` :5188）里的一个**长驻 claude 进程**（`claude --resume <sid>` 常驻，走 Max 5h 桶）。MCP 工具列表是这个 claude 进程**启动那一刻**一次性从 server 拉的快照。后加工具 + 重启 exec-mcp，长进程根本不会重新拉 → 它手里永远是旧列表。

排查关键：长 claude 进程的 STARTED 时间 < `server.py` 改动时间 = 它没见过新工具。
```bash
pgrep -af '/usr/local/bin/claude'        # 长进程启动时间
stat -c %y /root/mcp-server/server.py    # 对比工具改动时间
curl -s http://127.0.0.1:5188/state      # daemon 当前 session_id / pid / alive
```

**修法**: 重启 daemon，让它重新 spawn claude → 重连 MCP 拉新列表。session_id 存在 `/var/lib/claude-chat-daemon/state.json`，重启会自动 `--resume` 同一个 → 上下文不丢、不触发"刚醒失忆"（见 #16）。
```bash
systemctl restart claude-chat-daemon
curl -s http://127.0.0.1:5188/state      # 确认 alive:true 且 session_id 没变
```

**教训**: 改/加 MCP server 工具后，要想到**三层**都可能要动：① `exec-mcp`（server 进程本身，必重启）② chat 网页端 `ALLOWED_TOOLS` 白名单（`/opt/memory-v2/server.py`）③ **tg 端长 claude 进程（`claude-chat-daemon`，最容易漏掉的一层）**。长进程在启动时刻"快照"了工具列表，任何加工具都得 restart 它才生效。验证就看 `/state` 的 session_id 重启前后是否一致——一致才说明上下文续上了。
